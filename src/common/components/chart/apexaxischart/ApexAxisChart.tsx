...
import ApexAxisChartProps, { ApexAxisChartType } from "./ApexAxisChartProps";
...
const ApexAxisChart = (props: ApexAxisChartProps) => {
  const {
    chartTitle,
    apexChartType,
    apexChartOptions,
    apexChartSeries,
    apexAxisChartExportData,
  ...
  const [selectedChartType, setSelectedChartType] =
    useState<ApexAxisChartType>(apexChartType);

  const handleChartTypeSelectionChange = (
    event: SelectChangeEvent<unknown>
  ) => {
    setSelectedChartType(event?.target.value as ApexAxisChartType);
    charttypechangecallback(event?.target.value as ApexAxisChartType);
  };
  ...
  }, [blockZoomed, togglecallback, selectedChartType, apexAxisChartExportData]);
        ...
        <div className="fx-conduit-buttons tw-flex tw-items-center">
          <MuiButtonGroup className="tw-flex tw-items-center">
            ...
            >
              {apexAxisChartExportData && (
                <ReportDownload
                  formattedTableData={apexAxisChartExportData}
                  fileName={"VarTrendData"}
                />
              )}
            </MuiIconButton>
            <MuiIconButton
      ...
      <div className="fx-conduit-content tw-align-middle tw-h-[calc(100%-1.75rem)] tw-w-full tw-min-w-full tw-block">
        {apexChartOptions && apexChartSeries && (
          <ReactApexChart
            ...
            className="tw-w-full tw-min-w-full"
          />
        )}
