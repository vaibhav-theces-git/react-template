...
export interface BatchConfigCloneProps {
  isOpen: boolean;
  mode: BatchEditModes;
  ...
  batchCloneConfigurationSaveCallback: (batch: BatchConfiguration) => void;
  batchConfigurationCloseCallback: () => void;
}

const BatchConfigClone: React.FC<BatchConfigCloneProps> = (
  props: BatchConfigCloneProps
) => {
  const {
    isOpen,
    mode,
    ...
    legelEntityIdNames,
    batchVarConfigList,
    batchCloneConfigurationSaveCallback,
  ...
  const onDeleteEntity = () => {
    if (legelEntityTableData.some((e) => e.le_id === selectedEntity?.le_id)) {
      ...
      selectedEntityList.splice(
        selectedEntityList.findIndex((x) => x.le_id === selectedEntity?.le_id),
        1
      );
  ...
  const onSave = () => {
    if (!validateEmptyBatchName(batchName)) {
      ...
      if (
        ...
        batchCloneConfigurationSaveCallback(batchConfigToSave);
...
export default BatchConfigClone;
